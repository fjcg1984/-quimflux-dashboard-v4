...
function derive(r) {
  ...
  const merma = mp > 0 ? n(r.merma) / mp : null;
  ...
  const costoUnitario =
    q > 0 ? n(r.costo_produccion) / q : null;

  const energiaUnit =
    q > 0 ? n(r.energia) / q : null;
...
function renderDashboard() {
  ...
  const cards = [
    ['Producción total', producida.toLocaleString()],

    [
      'Cumplimiento',
      pct(cumplimiento),
      status(cumplimiento, metas.cumplimiento)
    ],

    [
      'Yield',
      pct(yieldRate),
      status(yieldRate, metas.yield)
    ],

    [
      'Merma',
      pct(mermaRate),
      status(mermaRate, metas.merma, true)
    ],

    [
      'Disponibilidad',
      pct(disponibilidad),
      status(disponibilidad, metas.disponibilidad)
    ],

    [
      'Asistencia',
      pct(asistencia),
      status(asistencia, metas.asistencia)
    ],

    [
      'Rechazo calidad',
      pct(rechazo),
      status(rechazo, metas.rechazo, true)
    ],
    ...
    ['Costo producción', 'S/ ' + costo.toLocaleString()],
    ...
    [
      ...
      energia === null
        ? '—'
        : energia.toFixed(3) + ' kWh/unidad'
    ],

    [
      'Entregas a tiempo',
      pct(otif),
      status(otif, metas.otif)
    ],

    [
      'Incidentes SSOMA',
      String(incidentes),
      status(incidentes, metas.incidentes, true)
    ]
  ];
      ...
      <div class="titleRow">
        <div>
          ...
          <p>
            ...
            ${maintenanceRows.length} mantenimientos
          </p>
        </div>
