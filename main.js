...
function renderDashboard(){
  const d=rows.map(derive);const sums=(key)=>d.reduce((s,r)=>s+n(r[key]),0);const k={prod:sums('producida'),programada:sums('programada'),mp:sums('mp'),mermaKg:sums('merma'),stop:sums('horas_paradas'),hours:sums('horas_turno'),pp:sums('personal_programado'),pa:sums('personal_presente'),rejKg:sums('rechazadas'),pedidos:sums('pedidos_programados'),aTiempo:sums('pedidos_tiempo'),reproceso:sums('reproceso'),nc:sums('no_conformidades'),cum:sums('programada')?sums('producida')/sums('programada'):0,yield:sums('mp')?sums('producida')/sums('mp'):0,merma:sums('mp')?sums('merma')/sums('mp'):0,disp:sums('horas_turno')?Math.max(0,(sums('horas_turno')-sums('horas_paradas'))/sums('horas_turno')):0,asis:sums('personal_programado')?sums('personal_presente')/sums('personal_programado'):0,rech:sums('producida')?sums('rechazadas')/sums('producida'):0,otif:sums('pedidos_programados')?sums('pedidos_tiempo')/sums('pedidos_programados'):0,costo:sums('costo_produccion'),mnt:sums('costo_mantenimiento'),unit:sums('producida')?sums('costo_produccion')/sums('producida'):0,energy:sums('producida')?sums('energia')/sums('producida'):0,inc:sums('incidentes')};k.oee=k.disp*k.cum*Math.max(0,1-k.rech);
  const status=(value,target,invert=false)=>{const ok=invert?value<=target:value>=target;const critical=invert?value>target*1.5:value<target*0.85;return {ok,critical,label:critical?'CRÍTICO':ok?'OK':'REVISAR',cls:critical?'critical':ok?'ok':'warn'};};const cards=[['Producción total',k.prod.toLocaleString()+' kg'],['Cumplimiento',pct(k.cum),status(k.cum,metas.cumplimiento)],['Yield',pct(k.yield),status(k.yield,metas.yield)],['Merma',pct(k.merma),status(k.merma,metas.merma,true)],['Disponibilidad',pct(k.disp),status(k.disp,metas.disponibilidad)],['Asistencia',pct(k.asis),status(k.asis,metas.asistencia)],['Rechazo calidad',pct(k.rech),status(k.rech,metas.rechazo,true)],['OEE',pct(k.oee),status(k.oee,.80)],['Costo producción','S/ '+k.costo.toLocaleString()],['Costo unitario','S/ '+k.unit.toFixed(3)],['Mantenimiento','S/ '+k.mnt.toLocaleString()],['Energía',k.energy.toFixed(3)+' kWh/kg'],['Entregas a tiempo',pct(k.otif),status(k.otif,metas.otif)],['Incidentes SSOMA',String(k.inc),status(k.inc,metas.incidentes,true)]];
  document.getElementById('content').innerHTML=`<main><div class="titleRow"><div><h1>Dashboard de Administración de Planta</h1><p>Datos sincronizados con Supabase · ${rows.length} registros</p></div><span class="online">● EN LÍNEA</span></div><div class="cards">${cards.map(c=>`<div class="card"><small>${c[0]}</small><strong>${c[1]}</strong>${c.length>2?`<span class="badge ${c[2].cls}">${c[2].label}</span>`:''}</div>`).join('')}</div><section class="panel"><h2>Últimos registros</h2>${d.length?`<div class="tableWrap"><table><thead><tr><th>Fecha</th><th>Turno</th><th>Producto</th><th>Programada</th><th>Producida</th><th>Merma</th><th>OEE</th></tr></thead><tbody>${d.slice(-20).reverse().map(r=>`<tr><td>${esc(r.fecha)}</td><td>${esc(r.turno)}</td><td>${esc(r.producto)}</td><td>${n(r.programada)}</td><td>${n(r.producida)}</td><td>${n(r.merma)}</td><td>${pct(r.oee)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">Todavía no hay registros. Ve a <b>Registro Diario</b> para ingresar el primero.</div>'}</section></main>`;
}
